...
module AppWeb
  ...
  def self.initialize!
    ...
    if !File.exists?(environment_file_path) 
      ...
      environment = AppWeb::DEFAULT_ENVIRONMENT
      environment_file_path = ::File.join(::File.expand_path('environments', self.config_dir), "#{AppWeb::DEFAULT_ENVIRONMENT}.rb")
    end
    ...
    puts "\tAppWeb loaded on environment \"#{ENV["RUBY_ENV"]}\" in #{Time.now - started_at} seconds"
    puts "#{"="*5} AppWeb loaded #{"="*40}"
