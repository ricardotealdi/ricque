...
module AppCore
  ...
  def self.initialize!
    ...
    if !File.exists?(environment_file_path) 
      ...
      environment = AppCore::DEFAULT_ENVIRONMENT
      environment_file_path = ::File.join(::File.expand_path('environments', self.config_dir), "#{AppCore::DEFAULT_ENVIRONMENT}.rb")
    end
    ...
    puts "\tAppCore loaded on environment \"#{ENV["RUBY_ENV"]}\" in #{Time.now - started_at} seconds"
    puts "#{"="*5} AppCore loaded #{"="*40}"
