...
module AppWorkers
  ...
  def self.initialize!
    ...
    if !File.exists?(environment_file_path) 
      ...
      environment = AppWorkers::DEFAULT_ENVIRONMENT
      environment_file_path = ::File.join(::File.expand_path('environments', self.config_dir), "#{AppWorkers::DEFAULT_ENVIRONMENT}.rb")
    end
    ...
    puts "\tAppWorkers loaded on environment \"#{ENV["RUBY_ENV"]}\" in #{Time.now - started_at} seconds"
    puts "#{"="*5} AppWorkers loaded #{"="*40}"
